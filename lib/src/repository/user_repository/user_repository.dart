...
class UserRepository extends GetxController {
  ...
  Future<void> createUser(UserModel user) async {
    ...
    while (await userSheet.values.map.rowByKey(json["Id"]) != null) {
      json = user.toJsonG();
    }
    await userSheet.values.map.appendRow(json);
  }
  ...
  Future<List<UserModel>> getAllUsers() async {
    ...
    final snapshot = await userSheet.values.map.allRows();
  ...
  Future<void> updateUserRecord(UserModel user) async {
    ...
    final cellsOfRow = await userSheet.cells.rowByKey(user.id.toString());
    ...
    await userSheet.cells.insert(cellsOfRow);
  }
  ...
  Future<bool> deleteUser(String id) async {
    ...
    final index = await userSheet.values.rowIndexOf(id);
    return index == -1 ? Future<bool>.value(false) : userSheet.deleteRow(index);
  }
}
